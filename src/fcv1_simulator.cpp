...
/// \param[in] filepath The path to the JSON configuration file.
...
/// \brief Convert internal stone data to a Python numpy array.
/// \param[in] simulated_stones The internal 16-slot stone positions after simulation.
/// \param[in] stones_per_team_out The number of stones per team to include in the output.
/// \returns A numpy array with shape (2, stones_per_team_out, 2).
...
/// \brief Handle a Box2D post-solve contact event.
/// \param[in] contact The contact pair solved by Box2D.
/// \param[in] impulse The impulse generated by the contact solver.
...
/// \brief Add an ID to a list only if it is not already present.
/// \param[in,out] list The list that stores unique IDs.
...
/// \brief Create a simulator instance and initialize Box2D stone bodies.
/// \param[in] stones The stone positions used as the pre-shot simulation state.
...
/// \brief Set the total shot count for the current simulation.
/// \param[in] total_shot The number of shots already thrown in the end.
void SimulatorFCV1::change_shot(int total_shot)
{
    this->total_shot = total_shot;
}

/// \brief Check whether a stone body is in the free guard zone.
/// \param[in] body The Box2D body of the stone to check.
/// \returns True if the stone is in the free guard zone; otherwise false.
...
/// \brief Check whether a stone has been removed from play.
/// \param[in] stone_id The internal stone ID to check.
/// \returns True if the stone is disabled, out of bounds, at the removed position, or hogged.
...
/// \brief Restore all stones to their pre-shot positions and stopped state.
...
/// \brief Record opponent stones protected by the five-rock free guard zone rule before the shot.
...
// ファイブロックルール対応用関数
/// \brief Apply the five-rock rule by restoring stones if a protected guard is removed.
...
// ノーティックルール対応用関数
/// \brief Check whether a stone body overlaps the center line.
/// \param[in] body The Box2D body of the stone to check.
/// \returns True if the stone is on the center line; otherwise false.
...
/// \brief Record opponent center guards protected by the no-tick rule before the shot.
...
// ノーティックルール対応用関数
/// \brief Apply the no-tick rule by restoring stones if a protected center guard is ticked or removed.
...
/// \brief Record stones protected by the mixed doubles modified free guard zone rule before the shot.
...
/// \brief Apply the modified free guard zone rule by restoring stones after a violation.
...
/// \brief Advance the physics simulation until all active stones stop.
/// \param[in] seconds_per_frame The time step size in seconds.
/// \returns The recorded stone trajectories for each simulation step.
...
/// \brief Initialize Box2D bodies from the stored stone positions.
...
/// \brief Set the thrown stone velocity and prepare rule-specific protected stones.
/// \param[in] velocity_x The x component of the thrown stone velocity.
/// \param[in] velocity_y The y component of the thrown stone velocity.
...
/// \brief Collect final stone positions after simulation and apply post-shot rule checks.
/// \returns The final internal 16-slot stone data.
...
/// \brief Create a Python-facing stone simulator wrapper.
...
/// \param[in] x_velocity The x component of the velocity of the stone to be thrown
/// \param[in] y_velocity The y component of the velocity of the stone to be thrown
